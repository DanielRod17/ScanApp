...
NSInteger *flagi;
@interface EPLReceipt () <UIGestureRecognizerDelegate>

@end

@implementation EPLReceipt
...
- (void)viewDidLoad {
    ...
    [linea connect];    [self.view addGestureRecognizer: [[UITapGestureRecognizer alloc] initWithTarget:self.view action:@selector(endEditing:)]];
    /////////////////////////////// (SIZE)
    NSString *URLmo = [NSString stringWithFormat:@"https://eplserver.net/erp/tools/BoxPalletID/Token&.php?CustomerReceipt="];
    URLmo = [URLmo stringByAppendingString:Order];
    NSURL *targetURL = [NSURL URLWithString:URLmo];
    NSURLRequest *requist = [NSURLRequest requestWithURL:targetURL];
    NSData *data = [NSURLConnection sendSynchronousRequest:requist returningResponse:nil error:nil];
    NSString *dataString = [[NSString alloc] initWithData:data encoding:NSASCIIStringEncoding];
    NSArray* fo = [dataString componentsSeparatedByString: @"|"];
    NSString* Customer = [fo objectAtIndex: 0];
    NSString* Att1 = [fo objectAtIndex: 1];
    NSString* Att2 = [fo objectAtIndex: 2];
    NSString* Att3 = [fo objectAtIndex: 3];
    Fixed1 = [fo objectAtIndex: 4];
    Fixed2 = [fo objectAtIndex: 5];
    Fixed3 = [fo objectAtIndex: 6];
    CAGradientLayer *gradientLayer = [EPLReceipt greyGradient];
    gradientLayer.frame = CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height);
    [self.view.layer insertSublayer:gradientLayer atIndex:0];
    self.Scrollito.contentSize =CGSizeMake(320, 1200);
    ...
    if(![Att3 isEqualToString:@"No"]){
        ...
        if(![Att2 isEqualToString:@"No"]){
            ...
            At3Name.text=Att3;
            UITextField *At3 = [[UITextField alloc] initWithFrame:CGRectMake(-90, 352, 225, 30)];
    ...
    if(second==1&&third!=1){
        ...
        CGRect FinishFrame = _FinishBut.frame;
        FinishFrame.origin.x = 53;
        FinishFrame.origin.y = 522;
        _FinishBut.frame = FinishFrame;
...
- (IBAction)Enviar:(id)sender {
    ...
    if([equalito isEqualToString:@"Success"]){
        EPLAppDelegate *appDelegate = (EPLAppDelegate *)[[UIApplication sharedApplication]delegate];
        Order = appDelegate.OrderID;
        Token = appDelegate.tokenReceipt;
        Usuarillo = appDelegate.Usuarillo;
        UITextField *attrib1 = (UITextField *)[self.view viewWithTag:101];
        UITextField *attrib2 = (UITextField *)[self.view viewWithTag:102];
        UITextField *attrib3 = (UITextField *)[self.view viewWithTag:103];
        ...
        Order= [Order stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        Usuarillo= [Usuarillo stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        ...
        NSString *URLmao = [NSString stringWithFormat:@"https://eplserver.net/erp/tools/BoxPalletID/Token&.php?SendReceiptPallet=1&PalletID="];
        ...
        URLmao = [URLmao stringByAppendingString:@"&Customer="];
        URLmao = [URLmao stringByAppendingString:Order];
        URLmao = [URLmao stringByAppendingString:@"&Tokenimo="];
        URLmao = [URLmao stringByAppendingString:Token];
        URLmao = [URLmao stringByAppendingString:@"&User="];
        URLmao = [URLmao stringByAppendingString:Usuarillo];
        NSURL *targetURL = [NSURL URLWithString:URLmao];
        NSURLRequest *requost = [NSURLRequest requestWithURL:targetURL];
        NSData *data = [NSURLConnection sendSynchronousRequest:requost returningResponse:nil error:nil];
        NSString *dataString = [[NSString alloc] initWithData:data encoding:NSASCIIStringEncoding];
        ...
        NSString* equalito = [foo objectAtIndex: 0];
        NSString* title;
        NSString* pallets = [foo objectAtIndex: 1];
        pallets = [pallets stringByAppendingString:@" Total Pallets"];
        if([equalito isEqualToString:@"Exists"])
        {
            ...
            title = @"Wrong PalletID";
        }
        UIAlertView * alert = [[UIAlertView alloc] initWithTitle:title message:pallets delegate:nil cancelButtonTitle:@"Close" otherButtonTitles:nil];
...
- (IBAction)Finish:(id)sender {
    ...
    NSString* equalito = [foo objectAtIndex: 0];
    if([equalito isEqualToString:@"Success"]){
        UIAlertView * alert = [[UIAlertView alloc] initWithTitle:@"Finish the Order" message:@"Are you sure?" delegate:self cancelButtonTitle:@"Yes" otherButtonTitles:@"No",nil];
        alert.tag=201;
        alert.alertViewStyle = UIAlertViewStyleDefault;
        [alert show];
        [alert release];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    if(alertView.tag==201){
        ...
        {
            UIAlertView * alert = [[UIAlertView alloc] initWithTitle:@"Sure?" message:@"" delegate:self cancelButtonTitle:@"Yes" otherButtonTitles:@"No",nil];
            alert.tag=200;
            alert.alertViewStyle = UIAlertViewStyleDefault;
            [alert show];
            [alert release];
        ...
        if(buttonIndex == 0)//OK button pressed
        {
            ...
            Usuarillo= [Usuarillo stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
            Token= [Token stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
            NSString *URLmao = [NSString stringWithFormat:@"https://eplserver.net/erp/tools/BoxPalletID/Token&.php?FinishReceipt=1&Tokencin="];
            URLmao = [URLmao stringByAppendingString:Token];
            URLmao = [URLmao stringByAppendingString:@"&Customercin="];
            URLmao = [URLmao stringByAppendingString:Order];
            URLmao = [URLmao stringByAppendingString:@"&Usercillo="];
            URLmao = [URLmao stringByAppendingString:Usuarillo];
            NSURL *targetURL = [NSURL URLWithString:URLmao];
            NSURLRequest *requost = [NSURLRequest requestWithURL:targetURL];
            NSData *data = [NSURLConnection sendSynchronousRequest:requost returningResponse:nil error:nil];
            ...
            NSArray* foo = [dataString componentsSeparatedByString: @"|"];
            NSString* equalito = [foo objectAtIndex: 0];
            NSString* totalillo = [foo objectAtIndex: 1];
            if([equalito isEqualToString:@"Exists"])
            {
                UIAlertView * alert = [[UIAlertView alloc] initWithTitle:@"Done" message:totalillo delegate:self cancelButtonTitle:@"Close" otherButtonTitles:nil];
                alert.tag=203;
                alert.alertViewStyle = UIAlertViewStyleDefault;
                [alert show];
                [alert release];
            ...
            {
                UIAlertView * alert = [[UIAlertView alloc] initWithTitle:@"No Pallets" message:@"No Records Found" delegate:nil cancelButtonTitle:@"Close" otherButtonTitles:nil];
                alert.tag=204;
                alert.alertViewStyle = UIAlertViewStyleDefault;
                [alert show];
                [alert release];
